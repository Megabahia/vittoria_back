...
from apps.MDP.mdp_parametrizaciones.serializers import ParametrizacionesSerializer,ParametrizacionesHijoSerializer,ParametrizacionesListaSerializer,ParametrizacionesFiltroSerializer,ParametrizacionesTipoSerializer
...
#logs 
from apps.ADM.vittoria_logs.methods import createLog,datosParametrizacionesMDP,datosTipoLog
#declaracion variables log
datosAux=datosParametrizacionesMDP()
...
#GET ALERT ABASTECIMIENTOS
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def alerta_abastecimiento_list(request):

    if request.method == 'GET':
        try:
            query= Parametrizaciones.objects.filter(state=1,tipo="ALERTA_ABASTECIMIENTO")
            serializer = ParametrizacionesFiltroSerializer(query, many=True)
            return Response(serializer.data)
        except Exception as e: 
            err={"error":'Un error ha ocurrido: {}'.format(e)}  
