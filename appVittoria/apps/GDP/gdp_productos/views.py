...
import openpyxl
# logs
from ...ADM.vittoria_logs.methods import createLog, datosTipoLog, datosProductosGDP

# declaracion variables log
datosAux = datosProductosGDP()
...
def producto_images_delete(request, pk):
    ...
    try:
        try:
            query = ProductoArchivos.objects.get(pk=pk, state=1).delete()
        except ProductoArchivos.DoesNotExist:
            err = {"error": "No existe"}
            createLog(logModel, err, logExcepcion)
            return Response(err, status=status.HTTP_404_NOT_FOUND)
        # tomar el dato
...
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def productos_findOne_codigoProducto(request, pk):
    timezone_now = timezone.localtime(timezone.now())
    logModel = {
        'endPoint': logApi + 'listOne/',
        'modulo': logModulo,
    ...
    try:
        try:
            query = Productos.objects.get(codigo=pk, state=1)
        except Productos.DoesNotExist:
            err = {"error": "No existe"}
            createLog(logModel, err, logExcepcion)
            return Response(err, status=status.HTTP_404_NOT_FOUND)
