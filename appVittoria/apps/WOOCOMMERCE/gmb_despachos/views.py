    ...
    if request.method == 'POST':
        try:
            ...
            if serializer.is_valid():
                ...
                dataPedidos = {**serializer.data, "estado": 'Pendiente',
                               "numeroPedido": serializer.data['numeroDespacho'], 'gestion_pedido': gestion_pedido,
                               'comprobanteVendedorGmb': gmb.archivoMetodoPago}
...
def megabahia_list(request):
    ...
    if request.method == 'POST':
        try:
            ...
            suma_total = Megabahia.objects.filter(**filters).aggregate(Sum('total'))

            serializer = MegabahiaSerializer(query[offset:limit], many=True)
...
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def megabahia_listOne(request, pk):
    timezone_now = timezone.localtime(timezone.now())
    logModel = {
    ...
    try:
        try:
            query = Megabahia.objects.get(pk=pk, state=1)
        except Megabahia.DoesNotExist:
            err = {"error": "No existe"}
            createLog(logModel, err, logExcepcion)
            return Response(err, status=status.HTTP_404_NOT_FOUND)
        # tomar el dato
        if request.method == 'GET':
            serializer = MegabahiaSerializer(query)
            createLog(logModel, serializer.data, logTransaccion)
...
@api_view(['POST'])
@permission_classes([IsAuthenticated])
def gmb_validate_megabahia(request):
        try:
...
@api_view(['POST'])
@permission_classes([IsAuthenticated])
def megabahia_update(request, pk):
    request.POST._mutable = True
    timezone_now = timezone.localtime(timezone.now())
    logModel = {
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if 'numeroComprobante' in request.data and request.data['numeroComprobante'] is not None:
                if Megabahia.objects.filter(numeroComprobante=request.data['numeroComprobante']).exclude(pk=pk).first():
            ...
            if 'numTransaccionTransferencia' in request.data and request.data['numTransaccionTransferencia'] is not None:
                if Megabahia.objects.filter(numTransaccionTransferencia=request.data['numTransaccionTransferencia']).exclude(pk=pk).first():
            ...
            if 'numTransaccionCredito' in request.data and request.data['numTransaccionCredito'] is not None:
                if Megabahia.objects.filter(numTransaccionCredito=request.data['numTransaccionCredito']).exclude(pk=pk).first():
            ...
            serializer = MegabahiaSerializer(query, data=request.data, partial=True)
