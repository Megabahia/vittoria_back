class MDMRouter:
    ...
    route_app_labels = {'mdm_parametrizaciones', 'mdm_clientes', 'mdm_facturas', 'mdm_negocios',
                        'mdm_prospectosClientes', 'mdm_contactos', 'mdm_asesores_comerciales'}
...
class MDPRouter:
    ...
    route_app_labels = {'mdp_parametrizaciones', 'mdp_categorias', 'mdp_productos', 'mdp_fichaTecnicaProductos',
                        'mdp_subCategorias', 'mdp_gestionInventario'}
...
class USERRouter:
    ...
    auth and contenttypes applications.
    """
    route_app_labels = {'vittoria_users'}

    def db_for_read(self, model, **hints):
        """
        Attempts to read auth and contenttypes models go to vittoria_users_db.
        """
        if model._meta.app_label in self.route_app_labels:
            return 'vittoria_users_db'
        return None

    def db_for_write(self, model, **hints):
        """
        Attempts to write auth and contenttypes models go to vittoria_users_db.
        """
        if model._meta.app_label in self.route_app_labels:
            return 'vittoria_users_db'
        return None
...
class FACTURACIONRouter:
    ...
    auth and contenttypes applications.
    """
    route_app_labels = {'facturacion'}

    def db_for_read(self, model, **hints):
        """
        Attempts to read auth and contenttypes models go to vittoria_facturacion_db.
        """
        if model._meta.app_label in self.route_app_labels:
            return 'vittoria_facturacion_db'
        return None

    def db_for_write(self, model, **hints):
        """
        Attempts to write auth and contenttypes models go to vittoria_facturacion_db.
        """
        if model._meta.app_label in self.route_app_labels:
            return 'vittoria_facturacion_db'
        return None
    ...
    def allow_migrate(self, db, app_label, model_name=None, **hints):
        """
        Make sure the auth and contenttypes apps only appear in the
        'vittoria_facturacion_db' database.
        """
        if app_label in self.route_app_labels:
            return db == 'vittoria_facturacion_db'
        return None
...
class MPRouter:
    ...
    auth and contenttypes applications.
    """
    route_app_labels = {'mp_parametrizaciones', 'woocommerce','gdc_gestor_contactos', 'gsb_pedidos', 'gmb_despachos', 'gd_parametrizaciones'}

    def db_for_read(self, model, **hints):
        """
        Attempts to read auth and contenttypes models go to vittoria_mp_db.
        """
        if model._meta.app_label in self.route_app_labels:
            return 'vittoria_mp_db'
        return None

    def db_for_write(self, model, **hints):
        """
        Attempts to write auth and contenttypes models go to vittoria_mp_db.
        """
        if model._meta.app_label in self.route_app_labels:
            return 'vittoria_mp_db'
        return None
    ...
    def allow_migrate(self, db, app_label, model_name=None, **hints):
        """
        Make sure the auth and contenttypes apps only appear in the
        'vittoria_mp_db' database.
        """
        if app_label in self.route_app_labels:
            return db == 'vittoria_mp_db'
        return None
